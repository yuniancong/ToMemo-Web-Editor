...
import { createCategory, createNote, createNotesInDisplayOrder, deleteCategory, deleteNotes, duplicateConflict, moveNotes, reorderCategoryNotes, updateNote } from './domain/editor'
...
export default function App() {
  ...
  function applyManualOrder(targetIndex: number, draggedId: string) {
    if (!config || !categoryId || readOnly) return
    const current = config.notes.filter((item) => item.categoryId === categoryId)
    const movingIds = selected.has(draggedId) ? current.filter((item) => selected.has(item.id)).map((item) => item.id) : [draggedId]
    const beforeCount = current.slice(0, targetIndex).filter((item) => !movingIds.includes(item.id)).length
    const remaining = current.filter((item) => !movingIds.includes(item.id)).map((item) => item.id)
    remaining.splice(beforeCount, 0, ...movingIds)
    commit(reorderCategoryNotes(config, categoryId, remaining))
    setSort('original'); setAscending(true); setDropIndex(null)
  }
  ...
  return <main className="app-shell" onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key.toLowerCase() === 'a' && config && !isTextEditingTarget(e.target)) { e.preventDefault(); setSelected(new Set(notes.map((item) => item.id))) } }}>
    ...
    <section className={`workspace-grid ${dragging ? 'dragging' : ''}`} {...drop}>
      ...
      <section className="memo-pane panel">
        ...
        {selected.size>1&&<div className="batch-bar"><strong>已选择 {selected.size} 项</strong><button onClick={()=>batchMove(false)}><Move size={13}/> 移动</button><button onClick={()=>batchMove(true)}><Copy size={13}/> 复制</button><button onClick={batchAffix}>标题处理</button><button onClick={exportSelected}>导出 AI 包</button><button className="danger" onClick={batchDelete}><Trash2 size={13}/> 删除</button></div>}
      </section>
