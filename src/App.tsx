...
export default function App() {
  ...
  async function loadFile(file: File) {
    const result = parseConfiguration(await file.text()); setError(null)
    if (!result.ok) { setError(result.errors.join('；')); return }
    setWorkspace({ fileName: file.name, configuration: result.configuration, warnings: result.warnings }); setPast([]); setFuture([]); setSelected(new Set())
  ...
  return <main className="app-shell" onKeyDown={(e) => { if ((e.metaKey || e.ctrlKey) && e.key === 'a' && config) { e.preventDefault(); setSelected(new Set(notes.map((item) => item.id))) } }}>
    ...
    {!workspace ? <section className={`empty-state ${dragging ? 'dragging' : ''}`} {...drop}><div className="empty-icon"><UploadCloud size={30}/></div><h1>选择一份 ToMemo 配置开始</h1><p>导入手机端导出的 JSON 文件。解析和编辑均在当前浏览器本地完成。</p><button className="button primary large" onClick={() => fileRef.current?.click()}><FolderOpen size={17}/> 选择 JSON 文件</button><div className="drop-hint">也可以将文件拖放到这里</div><div className="privacy-note"><ShieldCheck size={14}/> 文件不会上传到服务器</div></section> :
